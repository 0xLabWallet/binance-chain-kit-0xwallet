...
package com.wallet0x.binancechainkit.helpers;
...
import com.wallet0x.hdwalletkit.ECDSASignature;
import com.wallet0x.hdwalletkit.ECException;
import com.wallet0x.hdwalletkit.ECKey;

import static com.wallet0x.hdwalletkit.ECKey.HALF_CURVE_ORDER;
...
public class Crypto {
    ...
    public static byte[] bigIntegerToBytes(BigInteger b, int numBytes) {
        checkArgument(b.signum() >= 0);
        checkArgument(numBytes > 0);
        byte[] src = b.toByteArray();
        byte[] dest = new byte[numBytes];
        boolean isFirstByteOnlyForSign = src[0] == 0;
        int length = isFirstByteOnlyForSign ? src.length - 1 : src.length;
        checkArgument(length <= numBytes);
